...
def fixxpath(root, xpath):
    ...
    fixed_xpath = "/".join(
        ["{%s}%s" % (namespace, e) for e in xpath.split("/")])
    return fixed_xpath
...
class InstantiateVAppXML(object):
    ...
    def _make_virtual_hardware(self, parent):
        vh = ET.SubElement(parent, "VirtualHardwareSection",
                           {'xmlns:q1': "http://www.vmware.com/vcloud/v0.8"})
    ...
    def _add_network_association(self, parent):
        return ET.SubElement(parent, 'NetworkAssociation',
                             {'href': self.net_href})
...
class VCloudNodeDriver(NodeDriver):
    ...
    def vdcs(self):
        ...
        if not self._vdcs:
            ...
            self._vdcs = [
                self._to_vdc(
                    self.connection.request(get_url_path(
                        i.get('href'))).object)
    ...
    def networks(self):
        ...
        for vdc in self.vdcs:
            ...
            networks.extend([
                network for network in res.findall(
    ...
    def ex_list_nodes(self, vdcs=None):
        ...
        for vdc in vdcs:
            ...
            vapps = [
                (i.get('name'), i.get('href')) for i in elms
        ...
        for network in network_elements:
            ...
            try:
                network_object = self.connection.request(
                    get_url_path(network.get('href'))).object
            except:
                continue
            ...
            nat_rules = network_object.findall(
                fixxpath(
                    network_object, 'Configuration/Features/NatService/'
                    'NatRule/PortForwardingRule'))
            rules = {}
            for nat_rule in nat_rules:
                external_ip = nat_rule.find(
                    fixxpath(nat_rule, 'ExternalIpAddress'))
                if external_ip is not None:
                    external_ip = external_ip.text
                external_port = nat_rule.find(
                    fixxpath(nat_rule, 'ExternalPort'))
                if external_port is not None:
                    external_port = external_port.text
                internal_ip = nat_rule.find(
                    fixxpath(nat_rule, 'InternalIpAddress'))
                if internal_ip is not None:
                    internal_ip = internal_ip.text
                internal_port = nat_rule.find(
                    fixxpath(nat_rule, 'InternalPort'))
    ...
    def list_images(self, location=None):
        ...
        app_images = self.connection.request(
            '/api/query?type=vAppTemplate&pageSize=500').object
        images.extend(app_images)
        #images.extend(media_images)
        images = [
            self._to_image(i) for i in images
            if i.get('name') and i.get('href')
        ]
    ...
    def create_node(self, **kwargs):
        ...
        instantiate_xml = InstantiateVAppXML(name=name,
                                             template=image.id,
                                             net_href=network,
                                             cpus=str(kwargs.get('ex_cpus',
                                                                 1)),
                                             memory=str(size.ram),
                                             password=password,
        ...
        res = self.connection.request('%s/action/instantiateVAppTemplate' %
                                      get_url_path(vdc.id),
    ...
    def _get_auth_token(self):
        if not self.token:
            ...
            org_list_url = get_url_path(
                next(
                    (link for link in body.findall(fixxpath(body, 'Link'))
                     if link.get('type') ==
    ...
    def _build_xmltree(self):
        ...
        if self.network is not None:
            ...
            network_config_section = ET.SubElement(instantionation_params,
                                                   'NetworkConfigSection')
            ET.SubElement(network_config_section, 'Info',
                          {'xmlns': 'http://schemas.dmtf.org/ovf/envelope/1'})
    ...
    def _add_network_association(self, parent):
        ...
        if self.vm_fence is None:
            fencemode = self.network.find(
                fixxpath(self.network, 'Configuration/FenceMode')).text
        else:
...
class VCloud_1_5_NodeDriver(VCloudNodeDriver):
    ...
    def list_locations(self):
        return [
            NodeLocation(id=self.connection.host,
        ...
        for vdc in vdcs:
            ...
            for entity_elem in entity_elems:
                if entity_elem.get('type') == \
                    ...
                    res = self.connection.request(path, headers=headers)
    ...
    def ex_undeploy_node(self, node):
        ...
        try:
            res = self.connection.request('%s/action/undeploy' %
                                          get_url_path(node.extra.get('href')),
    ...
    def _perform_power_operation(self, node, operation):
        res = self.connection.request(
            '%s/power/action/%s' %
            (get_url_path(node.extra.get('href')), operation),
            method='POST')
    ...
    def ex_set_control_access(self, node, control_access):
        ...
        for subject in control_access.subjects:
            ...
            if subject.id:
                ...
                res = self.ex_query(type=subject.type,
        ...
        self.connection.request('%s/action/controlAccess' %
                                get_url_path(node.extra.get('href')),
    ...
    def ex_get_metadata(self, node):
        ...
        """
        res = self.connection.request('%s/metadata' %
                                      (get_url_path(node.extra.get('href'))))
    ...
    def create_node(self, name, image, size, **kwargs):
        ...
        if self._is_node(image):
            ...
            vapp_name, vapp_href = self._instantiate_node(
                name, image, network_elem, vdc, ex_vm_network, ex_vm_fence,
                ex_clone_timeout)
        ...
        if ex_deploy:
            ...
            while True:
                try:
                    res = self.connection.request('%s/power/action/powerOn' %
                                                  get_url_path(vapp_href),
                                                  method='POST')
                    self._wait_for_task_completion(res.object.get('href'))
                    break
    ...
    def _instantiate_node(self, name, image, network_elem, vdc, vm_network,
        ...
        instantiate_xml = Instantiate_1_5_VAppXML(name=name,
                                                  template=image.id,
    ...
    def _clone_node(self, name, sourceNode, vdc, clone_timeout):
        ...
        res = self.connection.request('%s/action/cloneVApp' %
                                      get_url_path(vdc.id),
    ...
    def _validate_vm_names(names):
        if names is None:
            return
        hname_re = re.compile(
            '^(([a-zA-Z]|[a-zA-Z][a-zA-Z0-9]*)[\-])*([A-Za-z]|[A-Za-z][A-Za-z0-9]*[A-Za-z0-9])$'
        )  # NOQA
        for name in names:
            if len(name) > 15:
                raise ValueError('The VM name "' + name +
                                 '" is too long for the computer '
    ...
    def _validate_vm_memory(vm_memory):
        if vm_memory is None:
            ...
            raise ValueError('%s is not a valid vApp VM memory value' %
                             vm_memory)
    ...
    def _change_vm_names(self, vapp_or_vm_id, vm_names):
        ...
        for i, vm in enumerate(vms):
            ...
            res = self.connection.request('%s/guestCustomizationSection' %
                                          get_url_path(vm.get('href')))

            # Update GuestCustomizationSection
            res.object.find(fixxpath(res.object,
                                     'ComputerName')).text = vm_names[i]
    ...
    def _change_vm_cpu(self, vapp_or_vm_id, vm_cpu):
        ...
        for vm in vms:
            ...
            res = self.connection.request('%s/virtualHardwareSection/cpu' %
                                          get_url_path(vm.get('href')))
    ...
    def _change_vm_memory(self, vapp_or_vm_id, vm_memory):
        ...
        for vm in vms:
            ...
            res = self.connection.request('%s/virtualHardwareSection/memory' %
                                          get_url_path(vm.get('href')))
    ...
    def _add_vm_disk(self, vapp_or_vm_id, vm_disk):
        ...
        for vm in vms:
            ...
            res = self.connection.request('%s/virtualHardwareSection/disks' %
                                          get_url_path(vm.get('href')))
    ...
    def _change_vm_script(self, vapp_or_vm_id, vm_script):
        ...
        for vm in vms:
            ...
            try:
                res.object.find(fixxpath(res.object,
                                         'CustomizationScript')).text = script
            except Exception:
            ...
            if (admin_auto_logon_enabled_elem is not None
    ...
    def _get_network_href(self, network_name):
        ...
        if network_href is None:
            raise ValueError('%s is not a valid organisation network name' %
                             network_name)
        else:
            return network_href
    ...
    def _to_nodes(self, vapp):
        ...
        for vm in vapp.extra.get('vms', []):
            node_id = vm.get('id').rstrip('/').split('/')[
                -1]  #more friendly id
            node_extra = {
