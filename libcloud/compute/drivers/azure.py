...
class AzureNodeDriver(NodeDriver):
    ...
    def list_locations(self):
        ...
        """
        data = self._perform_get('/' + self.subscription_id + '/locations',
                                 Locations)
    ...
    def list_nodes(self, ex_cloud_service_name=None):
        ...
        if ex_cloud_service_name:
            return self.list_nodes_for_cloud_service(
    ...
    def list_nodes_for_cloud_service(self, ex_cloud_service_name):
        ...
        try:
            ...
            for node in nodes:
                ...
                node.extra[
                    'location'] = data.hosted_service_properties.location
            return nodes
        except IndexError:
            return []
    ...
    def ex_start_node(self,
        ...
        try:
            response = self._perform_post(
                self._get_deployment_path_using_name(ex_cloud_service_name,
                                                     _deployment_name) +
                '/roleinstances/' + _str(node.id) + '/Operations', start_txt)
    ...
    def ex_stop_node(self,
        ...
        try:
            response = self._perform_post(
                self._get_deployment_path_using_name(ex_cloud_service_name,
                                                     _deployment_name) +
                '/roleinstances/' + _str(node.id) + '/Operations',
                shutdown_txt)
    ...
    def create_node(self,
        ...
        if not ex_cloud_service_name:
            try:
                ...
                ex_cloud_service_name = name + 'mistio' + str(
                    random.randint(1, 1000))
                self.create_cloud_service(ex_cloud_service_name,
                                          location=location)
        ...
        if re.search("Win|SQL|SharePoint|Visual|Dynamics|DynGP|BizTalk", image,
                     re.I):
            ...
            endpoint = ConfigurationSetInputEndpoint(
                ...
                load_balanced_endpoint_set_name=None,
                enable_direct_server_return=False)
            machine_config = LinuxConfigurationSet(name, ex_admin_user_id,
                                                   password, False,
                                                   custom_data)
        ...
        if "ex_storage_service_name" in kwargs:
            ...
            ex_storage_service_name = re.sub(ur'[\W_]+',
                                             u'',
            ...
            if self._is_storage_service_unique(
                ...
                ex_storage_service_name = ex_storage_service_name + 'mistio' + str(
        ...
        disk_name = "{0}-{1}-{2}.vhd".format(ex_cloud_service_name, name,
                                             time.strftime("%Y-%m-%d"))
        ...
        if node_list == []:
            ...
            response = self._perform_post(
                ...
                AzureXmlSerializer.virtual_machine_deployment_to_xml(
                    ex_deployment_name, ex_deployment_slot, name, name,
                    machine_config, disk_config, 'PersistentVMRole',
                    network_config, None, None, size, None))
        else:
            _deployment_name = self._get_deployment(
                service_name=ex_cloud_service_name,
                deployment_slot=ex_deployment_slot).name
    ...
    def destroy_node(self,
        ...
        _deployment = self._get_deployment(service_name=ex_cloud_service_name,
                                           deployment_slot=ex_deployment_slot)
        ...
        if _server_deployment_count > 1:
            ...
            try:
                ...
                self._perform_cloud_service_delete(
                    self._get_hosted_service_path(ex_cloud_service_name))
            except:
                pass
    ...
    def create_cloud_service(self,
        ...
        response = self._perform_cloud_service_create(
            ...
            AzureXmlSerializer.create_hosted_service_to_xml(
                ...
                self._encode_base64(ex_cloud_service_name), description,
                location, None, extended_properties))
    ...
    def create_volume_snapshot(self):
        raise NotImplementedError('You cannot create snapshots of '
                                  'Azure VMs at this time.')

    def attach_volume(self):
        raise NotImplementedError('attach_volume is not supported '
                                  'at this time.')

    def create_volume(self):
        raise NotImplementedError('create_volume is not supported '
                                  'at this time.')

    def detach_volume(self):
        raise NotImplementedError('detach_volume is not supported '
                                  'at this time.')

    def destroy_volume(self):
        raise NotImplementedError('destroy_volume is not supported '
                                  'at this time.')
    ...
    def _to_node(self, data):
        ...
        for endpoint in data.instance_endpoints:
            endpoints[endpoint.name] = '%s:%s %s' % (
                endpoint.local_port, endpoint.public_port, endpoint.protocol)
        ...
        return Node(id=data.role_name,
                    name=data.role_name,
                    state=self.NODE_STATE_MAP.get(data.instance_status,
                                                  NodeState.UNKNOWN),
    ...
    def _get_deployment(self, **kwargs):
        ...
        response = self._perform_get(
            self._get_deployment_path_using_slot(_service_name,
                                                 _deployment_slot), None)
    ...
    def _get_cloud_service_location(self, service_name=None):
        ...
        res = self._perform_get(
            ...
            '?embed-detail=False', HostedService)
    ...
    def _is_storage_service_unique(self, service_name=None):
        ...
        _check_availability = self._perform_get(
            self._get_storage_service_path() + '/operations/isavailable/' +
            _str(service_name) + '', AvailabilityResponse)
    ...
    def _perform_request(self, request):
        try:
            response = self.connection.request(action=request.path,
                                               data=request.body,
    ...
    def send_request_body(self, connection, request_body):
        ...
        elif (not isinstance(connection, HTTPSConnection)
              and not isinstance(connection, httplib.HTTPConnection)):
    ...
    def _parse_response(self, response, return_type):
        ...
        return_type.
        '''
        return self._parse_response_body_from_xml_text(response.body,
                                                       return_type)
    ...
    def _get_child_nodes(self, node, tagName):
        return [
            childNode for childNode in node.getElementsByTagName(tagName)
    ...
    def _fill_scalar_list_of(self, xmldoc, element_type,
    ...
    def _get_deployment_path_using_name(self,
        ...
        return self._get_path(
            'services/hostedservices/' + _str(service_name) + '/deployments',
            deployment_name)
    ...
    def _get_deployment_path_using_slot(self, service_name, slot=None):
        return self._get_path(
            'services/hostedservices/' + _str(service_name) +
    ...
    def _get_role_path(self, service_name, deployment_name, role_name=None):
        return self._get_path(
            'services/hostedservices/' + _str(service_name) + '/deployments/' +
            deployment_name + '/roles', role_name)
    ...
    def random_password(self):
        ...
        random_lower = ''
        for i in range(8):
            random_lower += "abcdefghijklmnopqrstuvwxyz" [random.randint(
                0, 25)]
...
class AzureXmlSerializer():
    ...
    def create_storage_service_input_to_xml(service_name, description, label,
        ...
        return AzureXmlSerializer.doc_from_data(
            ...
            [('ServiceName', service_name), ('Description', description),
             ('Label', label), ('AffinityGroup', affinity_group),
             ('Location', location),
    ...
    def update_hosted_service_to_xml(label, description, extended_properties):
        return AzureXmlSerializer.doc_from_data(
            'UpdateHostedService',
            [('Label', label, AzureNodeDriver._encode_base64),
             ('Description', description)], extended_properties)
    ...
    def create_hosted_service_to_xml(service_name, label, description,
                                     ...
                                     extended_properties):
        return AzureXmlSerializer.doc_from_data(
            'CreateHostedService', [('ServiceName', service_name),
                                    ('Label', label),
                                    ('Description', description),
                                    ('Location', location),
    ...
    def create_deployment_to_xml(name, package_url, label, configuration,
        ...
        return AzureXmlSerializer.doc_from_data(
            'CreateDeployment',
            [('Name', name), ('PackageUrl', package_url),
             ('Label', label, AzureNodeDriver._encode_base64),
    ...
    def swap_deployment_to_xml(production, source_deployment):
        return AzureXmlSerializer.doc_from_data(
            'Swap', [('Production', production),
                     ('SourceDeployment', source_deployment)])

    @staticmethod
    def update_deployment_status_to_xml(status):
        return AzureXmlSerializer.doc_from_data('UpdateDeploymentStatus',
                                                [('Status', status)])
    ...
    def upgrade_deployment_to_xml(mode, package_url, configuration, label,
        ...
        return AzureXmlSerializer.doc_from_data(
            'UpgradeDeployment',
            [('Mode', mode), ('PackageUrl', package_url),
             ('Configuration', configuration),
             ('Label', label, AzureNodeDriver._encode_base64),
             ('RoleToUpgrade', role_to_upgrade),
             ('Force', force, _lower)], extended_properties)

    @staticmethod
    def rollback_upgrade_to_xml(mode, force):
        return AzureXmlSerializer.doc_from_data('RollbackUpdateOrUpgrade',
                                                [('Mode', mode),
                                                 ('Force', force, _lower)])
    ...
    def create_affinity_group_to_xml(name, label, description, location):
        return AzureXmlSerializer.doc_from_data(
            'CreateAffinityGroup',
            [('Name', name), ('Label', label, AzureNodeDriver._encode_base64),
    ...
    def os_image_to_xml(label, media_link, name, os, showingui, ispremium):
        return AzureXmlSerializer.doc_from_data('OSImage',
                                                [('Label', label),
    ...
    def data_virtual_hard_disk_to_xml(host_caching, disk_label, disk_name, lun,
        ...
        return AzureXmlSerializer.doc_from_data(
            'DataVirtualHardDisk',
            [('HostCaching', host_caching), ('DiskLabel', disk_label),
             ('DiskName', disk_name), ('Lun', lun),
    ...
    def disk_to_xml(has_operating_system, label, media_link, name, os):
        return AzureXmlSerializer.doc_from_data(
            'Disk', [('HasOperatingSystem', has_operating_system, _lower),
                     ('Label', label), ('MediaLink', media_link),
                     ('Name', name), ('OS', os)])
    ...
    def windows_configuration_to_xml(configuration):
        xml = AzureXmlSerializer.data_to_xml([
            ...
            ('EnableAutomaticUpdates', configuration.enable_automatic_updates,
             _lower), ('TimeZone', configuration.time_zone)
        ])
    ...
    def linux_configuration_to_xml(configuration):
        ...
        if configuration.ssh is not None:
            ...
            for key in configuration.ssh.public_keys:
                xml += '<PublicKey>'
                xml += AzureXmlSerializer.data_to_xml([('Fingerprint',
                                                        key.fingerprint),
                                                       ('Path', key.path)])
                xml += '</PublicKey>'
            xml += '</PublicKeys>'
    ...
    def network_configuration_to_xml(configuration):
        ...
        for endpoint in configuration.input_endpoints:
            ...
            xml += AzureXmlSerializer.data_to_xml([
                ...
                ('LocalPort', endpoint.local_port), ('Name', endpoint.name),
                ('Port', endpoint.port)
            ])
    ...
    def role_to_xml(availability_set_name, data_virtual_hard_disks,
        ...
        if availability_set_name is not None:
            xml += AzureXmlSerializer.data_to_xml([('AvailabilitySetName',
        ...
        if data_virtual_hard_disks is not None:
            ...
            for hd in data_virtual_hard_disks:
                ...
                xml += AzureXmlSerializer.data_to_xml([
                    ...
                    ('DiskLabel', hd.disk_label), ('DiskName', hd.disk_name),
                    ('Lun', hd.lun),
    ...
    def add_role_to_xml(role_name, system_configuration_set,
        ...
        xml = AzureXmlSerializer.role_to_xml(availability_set_name,
                                             data_virtual_hard_disks,
                                             network_configuration_set,
                                             os_virtual_hard_disk, role_name,
                                             role_size, role_type,
                                             system_configuration_set)
    ...
    def virtual_machine_deployment_to_xml(
            deployment_name, deployment_slot, label, role_name,
            system_configuration_set, os_virtual_hard_disk, role_type,
            network_configuration_set, availability_set_name,
            data_virtual_hard_disks, role_size, virtual_network_name):
        ...
        if virtual_network_name is not None:
            xml += AzureXmlSerializer.data_to_xml([('VirtualNetworkName',
...
class ConfigurationSetInputEndpoints(WindowsAzureData):
    def __init__(self):
        self.input_endpoints = _list_of(ConfigurationSetInputEndpoint,
                                        'InputEndpoint')
...
class HostedServiceProperties(WindowsAzureData):
    def __init__(self):
        ...
        self.extended_properties = _dict_of('ExtendedProperty', 'Name',
                                            'Value')
...
class AzureNodeLocation(NodeLocation):
    ...
    def __repr__(self):
        return (('<AzureNodeLocation: id=%s, name=%s, country=%s, '
                ...
                (self.id, self.name, self.country, self.driver.name, ','.join(
                    self.available_services), ','.join(
                        self.virtual_machine_role_sizes)))
